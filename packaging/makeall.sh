...
smbus="no" # set to 'no' to turn off warning
spidev="yes" # set to 'no' to turn off warning
...
if [ $spidev == "yes" ] && ! grep "spidev" $debcontrol &> /dev/null; then
    warning "if this library does not depend on spidev change the 'spidev' variable!" && FLAG=true
fi
